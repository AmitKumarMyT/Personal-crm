...
import { Dumbbell, Wallet, LayoutGrid, Loader2, ChevronRight, Terminal } from 'lucide-react';
...
export default function Home() {
  ...
  const [appMode, setAppMode] = useState<'launcher' | 'hyper' | 'expense'>('launcher');
  ...
  if (!user) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center space-y-8 bg-[#050505]">
      <div className="relative w-32 h-32 mb-4">
        <Dumbbell className="w-full h-full text-[#FF2D55] animate-pulse relative z-10" />
      ...
      <h1 className="text-5xl font-extrabold tracking-tighter sm:text-7xl font-heading uppercase italic">
        SUPER<span className="neon-text"> PROTOCOL</span>
      </h1>
      <p className="max-w-md text-gray-400 text-lg">
        Unified biological and financial management.<br/>
        Enter the system.
      </p>
            ...
            >
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-2xl">
                  <button 
                    ...
                    className="group relative overflow-hidden glass p-10 rounded-[40px] border-white/5 hover:border-[#FF2D55]/30 transition-all text-left"
                  >
                     <div className="relative z-10 space-y-6">
                       <Dumbbell className="w-12 h-12 text-[#FF2D55] group-hover:scale-110 transition-transform" />
                       <div>
                         <h3 className="text-2xl font-black uppercase tracking-tighter">Hyper Coach</h3>
                         <p className="text-gray-500 text-sm mt-2 font-medium">Biological growth & AI training plans.</p>
                       </div>
                     </div>
                  ...
                  >
                     <div className="relative z-10 space-y-6">
                       <Wallet className="w-12 h-12 text-[#FF2D55] group-hover:scale-110 transition-transform" />
                       <div>
                         <h3 className="text-2xl font-black uppercase tracking-tighter">Expense Engine</h3>
                         <p className="text-gray-500 text-sm mt-2 font-medium">Distributed ledger & split logic.</p>
                       </div>
                     </div>
                     <div className="absolute inset-0 bg-[#FF2D55]/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                  </button>
        ...
        <div className="glass px-2 py-2 rounded-[32px] border border-white/10 flex items-center gap-1 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          {[
            ...
            >
               ...
               <span className={`text-[10px] font-black uppercase tracking-widest ${appMode === item.id ? 'block' : 'hidden md:block'}`}>
                 {item.label}
               </span>
            </button>
          ))}
