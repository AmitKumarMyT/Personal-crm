...
Return structured JSON.`;
  ...
  // Transform splitWeights array back to object for the component
  if (data.splitWeights && Array.isArray(data.splitWeights)) {
    const weights: Record<string, number> = {};
    data.splitWeights.forEach((item: any) => {
      weights[item.handle] = item.weight;
    });
    data.splitWeights = weights;
  }
