...
import { 
  ...
  Trash2, Edit3, AlertTriangle, ShieldCheck, Wallet, ArrowUpRight, ArrowDownLeft, CheckCircle2, UserPlus, Loader2
} from 'lucide-react';
...
export default function ExpenseManager() {
  ...
  const processTransaction = async (txData: any, newContactHandles: string[] = []) => {
    ...
    const getShareFor = (handle: string) => {
      if (txData.splitWeights) {
        ...
        const totalWeight = Object.values(weights).reduce((a: any, b: any) => a + (Number(b) || 1), 0) as number;
        const myWeight = Number(weights[handle]) || 1;
        return (total * myWeight) / totalWeight;
      }
      return total / txData.involvedMembers.length;
    };
    ...
    return (
      <div className="space-y-8 animate-fade-in relative z-10 pb-20">
        <div className="flex flex-col md:flex-row gap-4 items-stretch">
          <GlassCard className="flex-1 border-green-500/20 bg-green-500/[0.02] flex items-center justify-between">
            <div>
              <div className="text-[10px] uppercase font-black tracking-widest text-green-400/60 mb-1">Receivables</div>
              <div className="text-3xl font-black tracking-tighter text-green-400">₹{totalOwedMe.toFixed(0)}</div>
            </div>
            <div className="p-3 bg-green-500/10 rounded-2xl">
              <TrendingUp className="w-6 h-6 text-green-400" />
            </div>
          </GlassCard>
          <GlassCard className="flex-1 border-[#FF2D55]/20 bg-[#FF2D55]/0.02 flex items-center justify-between">
            <div>
              <div className="text-[10px] uppercase font-black tracking-widest text-[#FF2D55]/60 mb-1">Payables</div>
              <div className="text-3xl font-black tracking-tighter text-[#FF2D55]">₹{totalIOwe.toFixed(0)}</div>
            </div>
            <div className="p-3 bg-[#FF2D55]/10 rounded-2xl">
              <TrendingDown className="w-6 h-6 text-[#FF2D55]" />
            </div>
          </GlassCard>
        </div>

        <div className="sticky top-0 z-20 py-4 bg-[#0a0a0a]/80 backdrop-blur-md rounded-2xl px-2">
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-1 group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 group-focus-within:text-[#FF2D55] transition-colors" />
              <input 
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                placeholder="Search protocol or handle..."
                className="w-full bg-white/5 border border-white/5 rounded-2xl pl-12 pr-4 py-4 text-sm focus:outline-none focus:border-[#FF2D55]/30 transition-all font-medium"
              />
            </div>
            <button 
              onClick={() => setShowAddPerson(true)}
              className="w-full md:w-auto px-6 py-4 bg-[#FF2D55] text-white rounded-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest shadow-[0_4px_20px_rgba(255,45,85,0.4)]"
            >
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-bold text-xl border transition-all ${contact.balance > 0 ? 'bg-green-500/10 border-green-500/20 text-green-400' : contact.balance < 0 ? 'bg-[#FF2D55]/10 border-[#FF2D55]/20 text-[#FF2D55]' : 'bg-white/5 border-white/10 text-gray-400'}`}>
                        {contact.name[0]}
                      </div>
                      <div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/5">
                    <div className="flex gap-1.5">
                      ...
                      <span className="text-[8px] font-black uppercase tracking-widest text-gray-500">Active Ledger</span>
                    </div>
                    <div className="flex gap-2">
                       <button className="p-2 bg-white/5 rounded-lg hover:bg-[#FF2D55]/20 hover:text-[#FF2D55] transition-all">
          ...
          <div className="lg:col-span-3 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
              <div className="space-y-6">
                ...
                <div className="space-y-10 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar">
  ...
  const renderChat = () => (
    <div className="flex flex-col h-[calc(100vh-200px)] animate-fade-in relative z-10 lg:px-4">
       <div className="flex-1 overflow-y-auto space-y-6 pb-20 pr-2 custom-scrollbar flex flex-col pt-4">
       ...
       <div className="mt-4 flex flex-col gap-4 relative">
         ...
         <AnimatePresence>
           {isAiProcessing && (
             ...
             >
               <Loader2 className="w-3 h-3 animate-spin" /> Calibrating Split Ledger
             </motion.div>
           )}
         </AnimatePresence>
  ...
  return (
    <div className="flex flex-col h-full bg-[#050505]">
      ...
      {!selectedPerson && (
        <header className="sticky top-0 z-50 bg-[#050505]/80 backdrop-blur-md border-b border-white/5 px-6">
          <div className="max-w-4xl mx-auto flex items-center justify-between py-4">
             <div className="flex items-center gap-3">
                ...
                <h1 className="text-sm font-black uppercase tracking-[2px]">Expense Engine</h1>
             </div>
             ...
             <nav className="flex gap-1 bg-white/[0.02] p-1 rounded-xl border border-white/5">
                {navigation.map((nav) => (
                  <button 
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === nav.id ? 'bg-[#FF2D55] text-white shadow-[0_4px_10px_rgba(255,45,85,0.3)]' : 'text-gray-500 hover:text-white'}`}
                  >
