        ...
        <header className="sticky top-0 z-50 bg-[#050505]/80 backdrop-blur-md border-b border-white/5 px-6">
          <div className="max-w-4xl mx-auto flex items-center justify-between py-4">
             <div className="flex items-center gap-3">
                ...
                <h1 className="text-sm font-black uppercase tracking-[2px] hidden md:block">Expense Engine</h1>
             </div>

             <nav className="flex gap-1 bg-white/[0.02] p-1 rounded-xl border border-white/5 overflow-x-auto no-scrollbar">
                {navigation.map((nav) => (
                  <button 
                    key={nav.id} 
                    onClick={() => setActiveTab(nav.id as any)} 
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === nav.id ? 'bg-[#FF2D55] text-white shadow-[0_4px_10px_rgba(255,45,85,0.3)]' : 'text-gray-500 hover:text-white'}`}
                  >
